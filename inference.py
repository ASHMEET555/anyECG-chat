...
parser.add_argument('--data_path', type=str, default=None, help='Path to custom data JSON file')
...
parser.add_argument('--batch_size', type=int, default=None, help='Alias for eval_batch_size')
...
# Handle argument aliases and overrides
if args.batch_size is not None:
    args.eval_batch_size = args.batch_size
if args.output_dir is not None:
    os.makedirs(args.output_dir, exist_ok=True)
    args.result_path = os.path.join(args.output_dir, 'results.json')
...
if args.model_name == 'anyECG-chat':
    ...
    def get_response(ecgs, messages):
        ...
        if isinstance(ecgs, list):
            ecgs = [torch.tensor(e, dtype=torch.float32).cuda() if not isinstance(e, torch.Tensor) else e.cuda() for e in ecgs]
        elif isinstance(ecgs, torch.Tensor):
            ecgs = ecgs.cuda()
...
def test_custom():
    ...
    # Data
    data_file = args.data_path if args.data_path else f'./data/{args.dataset}_test.json'
    dataset = load_dataset('json', data_files=data_file)['train']
    test_dataloader = DataLoader(dataset, batch_size=args.eval_batch_size, shuffle=False, collate_fn=Collate_Fn(), num_workers=8)
    # TEST
    count = 0
    samples_all = []
    for ecgs, messages in tqdm(test_dataloader):
        # Move ECGs to CUDA - handle list of tensors
        if isinstance(ecgs, list):
            ecgs = [ecg.cuda() if isinstance(ecg, torch.Tensor) else ecg for ecg in ecgs]
        else:
            ecgs = ecgs.cuda()
        question_message = [[item[0]] for item in messages]
        answer_message = [[item[1]] for item in messages]
